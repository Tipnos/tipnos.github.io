...
Named after Charles Clos, it is a type of non-blocking, multistage switching network architecture first described in 1953, designed to minimize the number of crosspoints while maintaining high connectivity. To keep things simple, a 2-tier Clos topology is used in this series of articles. At the time of writing, datacenters mostly use 3 tiers or more for large ones (like cloud providers).
...
>Note: in some EVPN literature "border gateway" is reserved for a node interconnecting two separate EVPN sites/fabrics (multi-site/DCI), while any fabric-edge node reaching an external network is a "border leaf". This series uses "border gateway" more loosely, for whatever device (router, firewall, DCI node) the border leaves hand traffic off to outside the fabric.
...
1. **Predictability**: the hop count (the number of devices the traffic passes through) for any leaf reaching any other leaf or border leaf is constant across the entire network, providing predictable performance. It's no longer true with a 3-tier topology: some paths (that cross between pods) gain two extra hops.
1. **Expandability**: the network grows naturally without requiring a large upfront investment or any later reconfiguration.
...
1. _Data plane traffic_:
   1. **Attached servers outbound**: Look up attached servers' packets and compute the destination, which is either a local tenant's server or another leaf. If it is another leaf, encapsulate the packet with its own VTEP IP as the source and the computed leaf's VTEP IP as the destination. _Protocols_: **BGP, VXLAN**.
...
1. _Data plane traffic_:
   1. **Fabric outbound**: Decapsulate packets from other leaves, perform a route look-up, and forward them to the proper gateway. _Protocols_: **BGP, VXLAN, VRF**
   1. **Fabric inbound**: Compute the destination leaf by performing a route look-up, and encapsulate the packets accordingly. _Protocols_: **BGP, VXLAN, VRF**
...
Also, two terms come up constantly in this space and are worth introducing now: the **underlay**, the plain IP network that gives every switch reachability to every other switch (no tenant awareness at all), and the **overlay**, the tenant-aware layer built on top of it that carries the actual L2/L3 reachability information and encapsulated tenant traffic. VXLAN alone only defines how tenant traffic is encapsulated over the underlay; by itself it says nothing about how switches learn *where* to send it, historically relying on flood-and-learn or multicast. EVPN ([RFC 7432](https://datatracker.ietf.org/doc/html/rfc7432), extended for VXLAN by [RFC 8365](https://datatracker.ietf.org/doc/html/rfc8365)) fills that gap by using BGP to distribute L2/L3 reachability as an overlay control plane, which is why it pairs so consistently with VXLAN in modern fabrics.
